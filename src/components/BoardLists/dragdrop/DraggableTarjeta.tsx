...
const DraggableTarjeta: React.FC<Props> = ({
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-between gap-1 flex-wrap">
          <div className="flex gap-1 flex-wrap">
            {getVisibleTags(card.tags).map((tag, idx) => (
              <div key={idx} className="rounded-[16px] bg-[#414141] text-[#E5E7EB] text-[11px] font-poppins px-3 py-0.5 w-fit">
          ...
          <div className="flex -space-x-2">
            {card.assignees && card.assignees.length <= 2
              ? card.assignees.map((user, idx) => (
                  <img key={idx} src={user.avatar_url} alt={user.name} className="w-6 h-6 rounded-full border-[0.5px] border-black" />
                ))
              : card.assignees && card.assignees.length > 2 && (
                  <>
      ...
      {showDeleteModal && <DeleteCardModal onCancel={() => setShowDeleteModal(false)} onConfirm={handleConfirmDelete} isDeleting={isDeleting} />}
    </>
  );
};
