...
export default function ReminderSelect() {
    ...
    const [selected, setSelected] = useState<string>("Ninguno");
    ...
    const handleAddOption = () => {
        if (customValue.trim() !== "" && !options.includes(customValue)) {
    ...
    return (
        <div className="relative">
            <button type="button" className="font-poppins w-full p-2 rounded-lg outline-none bg-[#2a2a2a] text-gray-400 border border-[#3a3a3a] text-left relative flex justify-between items-center"
                    ...
                    {options.map((option) => (
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <div
                                    className={`w-4 h-4 rounded-full border border-gray-400 flex items-center justify-center transition-colors duration-200 ${selected === option ? "bg-[#6a5fff] border-[#6a5fff]" : ""
                                        }`}
                                >
                                    {selected === option && (
